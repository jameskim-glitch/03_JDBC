...
public class UserView {
	...
	private void updateName() throws Exception {
		...
		//수정(UPDATE) 서비스 호출 수 결과 반환(INT) 받기
	...
	private void deleteUser() throws Exception {
		System.out.println("=== 5. USER_NO 를 입력받아 일치하는 USER 삭제===");
		...
		else		   System.out.println("사용자 번호가 일치하는 User 가 존재하지 않습니다.");
	...
	private void selectUser() throws Exception {
		...
		// -> 1행의 조회결과를 담기 위해서 User DTO 객체 1개 사용
		User user = service.selectUser(input);
		
		// 조회결과가 없으면 null / 있으면 !null
		if (user == null) {
			System.out.println("USER_NO 가 일치하는 회원 없음");
			return;
		} 
		System.out.println(user);
	}
	...
	private void selectAll() throws Exception {
		System.out.println("\n=== 2. User 전체 조회 ===");
		//2. 서비스 호출(SELECT) 후 결과 (List(User)반환받기
		List<User> userList =  service.selectAll();
		// 결과에 따라 처리하기
		
		// 조회 결과가 없을 경우
		if(userList.isEmpty()) { //userList 가 비어 있다면
			System.out.println("***조회 결과가 없습니다***");
			return;
			}
		// 조회 결과가 있을 경우 
		//향상된 for 문 이용해서 userList에 있는 user 객체 출력
