...
// (Model 중 하나)DAO (Data Acess Object)
...
public class UserDAO {
	 ...
	 * @return : 아이디가 일치하는 User 객체 또는 null
	 */
	public User selectId(Connection conn, String input) {
		...
		return null; // 결과 반환(생성된  User객체 또는 Null)
	}
	/** 1. User 등록 DAO
	...
	public int InsertUser(Connection conn, User user) throws Exception { 
		// SQL 수행중 발생하는 예외를
		// catch 로 처리하지 않고, throws를 이용해서 호출부로 던져 처리
		// -> catch 문 필요 없다
		...
		try {
			...
			// 5. SQL(INSERT) 수행 (excuteUpdate()) 후 결과 (삽입된 행의 갯수) 반환받기
	...
	public List<User> selectAll(Connection conn) throws Exception {
		...
		try {
			...
			// 5. SQL(SELECT) 수행(excuteQuery) 후 결과(ResultSet) 반환 받기
			rs = pstmt.executeQuery();
			
			// 6. 조회 결과를 1행 씩 접근하여 컬럼 값 얻어오기
			// 몇 행이 조회될지 모른다 -> While
			// 무조건 1행만 조회된다 -> if
			...
			while(rs.next()) {
				...
				String userPw = rs.getString("USER_PW");
				String userName = rs.getString("USER_NAME");
				String enrolDate = rs.getString("ENROLL_DATE");
				// java.sql.Date 타입으로 값을 저장하지 않은 이유
				// -> SELECT 문에서 TO_CHAR() 를 이용하여 문자열로 변환해 조회했기 때문
				
				// User 객체 새로 생성하여 컬럼값 세팅하기
				User user = new User(userNo, userId, userPw, userName, enrolDate);
	...
	/** 4. USER_NO 를 입력받아 일치한 User 조회 DAO
	 * @param conn
	 * @param input
	...
	public User selectUser(Connection conn, int input) throws Exception {
		...
		try {
			...
			if(rs.next()) {
				...
				user = new User(userNo, userId, userPw, userName, enrolDate);
	...
	public int idCheck(Connection conn, String userId) throws Exception {
		...
		try {
			...
			if(rs.next()) {
				count = rs.getInt(1); // 조회된 컬럼 순서 번호를 이용해
										// 컬럼값 얻어오기
			}
