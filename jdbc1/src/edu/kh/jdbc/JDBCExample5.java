...
public class JDBCExample5 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 장점 1. SQL 작성이 간단해짐
		 * 장점 2. ? 에 값 대입시 자료형에 맞는 형태의 리터럴 으로 대입됨!
		...
		// SELECT 가 아니기 때문에 ResultSet 은 필요가 없다
		
		Scanner sc = null;
		
		try {
			// 2. DriverManger를 이용해 Connection 객체 생성
            ...
            // + DML 수행전 해줘야 할 것! 
            // AutoCommit 끄기!
            // -> 왜 끄는 건가? 개발자가 트랜젝션을 마음대로 제어하기 위해서
            conn.setAutoCommit(false);
            
            // 6. SQL(INSERT) 수행 후 결과(int) 반환 받기
            // executeQuery() : SELECT 수행, ResultSet 반환
            // executeUpdate() : DML 수행, 결과행 갯수(int) 반환
            // -> 보통 DML 실패 0, 성공 시 0 초과된 값이 반환된다
            
            // psmt 에서 executeQuery() , executeUpdate() 매개변수 자리에 아무것도 없어야 한다!
            int result = pstmt.executeUpdate();
            
            // 7. result 값에 따른 결과 처리 + 트랜젝션 제어 처리
